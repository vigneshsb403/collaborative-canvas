...
| Draw offline | Kill the server, keep drawing, then restart it. The strokes you drew while it was down are re-sent and appear for everyone. |
...
Those tests earned their keep: the pixel assertions caught a real bug where undo
left an antialiased fringe behind, because bounding boxes are padded in world units
and that is less than one device pixel when the paper is displayed small.
